...
rsquared.from_returns <- function(desired_year, fund, alpha, beta) {
  ...
  return(alpha_beta_r(fund.actual.vals, alpha, beta, xs.curr$CROBEX, xs.curr$InterestRate.daily))
}

xs.capm$R.squared <- mapply(rsquared.from_returns, xs.capm$Year, xs.capm$Fund, xs.capm$Alpha, xs.capm$Beta)
xs.capm
...
anova.returns <- xs.returns %>% 
  ...
  unlist
factors <- anova.subset %>% 
  rep(rep(num.items, length(.))) %>% 
  as.factor

anova(lm(anova.returns ~ factors))
