...
get_capm_for_fund <- function(df, selected_fund){
  ...
  data.frame(selected_fund, CAPM.alpha(xts(fund[, -1], order.by=fund$Date), xts(capm.index[, -1], order.by=capm.index$Date), capm.risk_free[1, -1]), CAPM.beta(xts(fund[, -1], order.by=fund$Date), xts(capm.index[, -1], order.by=capm.index$Date), capm.risk_free[1, -1]))
}
...
ggplot(xs.capm.all, aes(Beta, color= Fund, group = Fund)) + 
  geom_point() + geom_line()
ggplot(xs.capm.all, aes(Alpha, color= Fund, group = Fund)) + 
  geom_point() + geom_line()
