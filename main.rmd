...
nrows = nrow(xs.capm)
#for(i in 1:361) {
#  xs.capm[as.character(i)] <- as.vector(matrix(0,nrow=nrows))
#}
#
xs.capm["ks_p_val"] <- as.vector(0)
get_residuals_norm <- function(df, row) {
  ...
  capm.risk_free.year <- xs.tmpYear$InterestRate.daily #[c('Date', 'InterestRate.daily')]
  ...
  for (i in 1:length(xs.tmpYearFund)) {
    ...
    (df[row,3] + df[row, 4] * (capm.market.year[i] - capm.risk_free.year[i])))^2
  }
  
  x<-ks.test(residuals.tmp, 'pnorm')
  x
  df[row, 'ks_p_val'] <- x$p.value
  
  return(df)
}

for(i in 1:nrows) {
  xs.capm <- get_residuals_norm(xs.capm, i)
}
