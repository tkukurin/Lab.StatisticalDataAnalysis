...
Iz ovih se grafova vidi kako su investicijski fondovi (prva tri stupca) na dnevnoj bazi podložniji većim promjenama vrijednosti od mirovinskih.
...
xs.2010 <- xs.returns[year(xs.returns$Date) == 2010, ]
capm.raiffeisen.2010 <- xs.capm[xs.capm$Year == 2010 & xs.capm$Fund == "RaiffeisenDMF", ]

alpha_beta_r(xs.2010$RaiffeisenDMF, 
             capm.raiffeisen.2010$Alpha, 
             capm.raiffeisen.2010$Beta, 
             xs.2010$CROBEX, 
             xs.2010$InterestRate.daily)

model.2010 <- lm((xs.2010$RaiffeisenDMF - xs.2010$InterestRate.daily) ~ (xs.2010$CROBEX - xs.2010$InterestRate.daily))
model.2010
capm.raiffeisen.2010$Alpha
capm.raiffeisen.2010$Beta
model.2010.summary <- summary(model.2010)
model.2010.summary$r.squared


iterate <- function(desired_year, fund, alpha, beta) { 
  xs.curr <- xs.returns[year(xs.returns$Date) == desired_year, ]
  
  return(alpha_beta_r(xs.curr[, as.character(fund)], 
                      alpha, 
                      beta, 
                      xs.curr$CROBEX, 
...
xs.capm$R.squared <- mapply(iterate, xs.capm$Year, xs.capm$Fund, xs.capm$Alpha, xs.capm$Beta)
xs.capm
