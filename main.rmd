...
capm.risk_free <- xs.returns$InterestRate
capm.market <- xs.returns$CROBEX
capm.observed <- xs.returns$ZBAktiv

capm.df <- data.frame(capm.risk_free, capm.market, capm.observed)
capm.df

lm(formula = capm.observed ~ I(capm.risk_free) + (capm.market - capm.risk_free), data = capm.df)
?I
```
...
zbaktiv.week <- to.weekly(xts(zbaktiv.2010[, -1], order.by=zbaktiv.2010$Date))
?to.weekly

quantile(zbaktiv.week)
